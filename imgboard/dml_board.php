...
if (isset($postAndget_mode) && $postAndget_mode == "insert") {
  ...
  if (empty($content) || empty($subject) || empty($code_content) || empty($language)) {
    alert_back('모두 입력해주세요!');
  }
  $subject = test_input($_POST["subject"]);
  $content = test_input($_POST["content"]);
  $code_content = test_input($_POST["code_content"]);
  $userid = test_input($userid);
  $hit = 0;
  $q_subject = mysqli_real_escape_string($con, $subject);
  $q_content = mysqli_real_escape_string($con, $content);
  $q_code_content = mysqli_real_escape_string($con, $code_content);
  ...
  $sql = "INSERT INTO `code` VALUES (null,'$q_userid','$q_subject','$q_content','$q_code_content','$language','$regist_day',0,'$upfile_name','$copied_file_name','$upfile_type');";
  ...
  $sql = "SELECT num from `code` where id ='$userid' order by num desc limit 1;";
  $result = mysqli_query($con, $sql);
  if (!$result) {
  ...
  $sql = "DELETE FROM `code` WHERE num=$q_num";
  $result = mysqli_query($con, $sql);
  if (!$result) {
    die('Error: ' . mysqli_error($con));
  }
  ...
  if (empty($content) || empty($subject) || empty($code_content)) {
    alert_back('모두 입력해주세요!');
  }
  $subject = test_input($_POST["subject"]);
  $content = test_input($_POST["content"]);
  ...
  if (isset($_POST['del_file']) && $_POST['del_file'] == '1') {
    ...
    $sql = "SELECT `file_copied_0` from `code` where num ='$postAndget_num';";
    $result = mysqli_query($con, $sql);
    if (!$result) {
      alert_back('Error: 6' . mysqli_error($con));
      // die('Error: ' . mysqli_error($con));
    ...
    $sql = "UPDATE `code` SET `file_name_0`='', `file_copied_0` ='', `file_type_0` =''  WHERE `num`=$postAndget_num;";
    $result = mysqli_query($con, $sql);
    if (!$result) {
      die('Error: ' . mysqli_error($con));
    }
  ...
  if (!empty($_FILES['upfile']['name'])) {
    ...
    $sql = "UPDATE `code` SET `file_name_0`= '$upfile_name', `file_copied_0` ='$copied_file_name', `file_type_0` ='$type[0]' WHERE `num`=$postAndget_num;";
    $result = mysqli_query($con, $sql);
    if (!$result) {
      die('Error: ' . mysqli_error($con));
    }
  ...
  $sql = "UPDATE `code` SET `subject`='$q_subject',`content`='$q_content',`code_content`='$q_code_content',`regist_day`='$regist_day'  WHERE `num`=$postAndget_num;";
  $result = mysqli_query($con, $sql);
  if (!$result) {
    die('Error: ' . mysqli_error($con));
  }
  ...
  isset($postAndget_mode) && $postAndget_mode
  == "insert_ripple"
) {
